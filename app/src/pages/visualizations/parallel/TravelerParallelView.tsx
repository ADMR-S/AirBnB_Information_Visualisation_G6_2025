...
export default function TravelerParallelView() {
  ...
  useEffect(() => {
    ...
    // draw axes on SVG (keep text crisp)
    const axis = svg
    ...
    // prepare canvas for drawing lines
  ...
  // Color scale by room_type (use legend arrays computed above)
  ...
  const groups = d3.group<AirbnbListing, string>(data, (d: AirbnbListing) => d.room_type);
    ...
    availRoomTypes.forEach((rt: string) => {
    ...
    // draw lines in batches to avoid freezing
  ...
  // --- Stats computation for the section below the chart ---
  ...
  // room type distribution for currently filtered set
  ...
  return (
    <div className="viz-container">
      ...
      {/* Controls + Legend for room types */}
      ...
      <div style={{ marginTop: 18, padding: 12, borderRadius: 8, background: 'rgba(250,250,250,0.9)', boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div style={{ minWidth: 260 }}>
            ...
            <div style={{ fontSize: 13, marginBottom: 6 }}>Total affiché (room types actifs): <strong>{totalShown.toLocaleString()}</strong></div>
